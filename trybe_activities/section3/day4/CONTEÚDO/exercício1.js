...
// 2 - Adicione um novo livro favorito na chave favoriteBooks, que é um array de objetos. Atribua a essa chave um objeto que contenha as seguintes informações:
                ...
                //   publisher: 'Rocco',
                // }
let favoritesArray = reader.favoriteBooks;
favoritesArray.push( {
  title: 'Harry Potter e o Prisioneiro de Azkaban',
  author: 'JK Rowling',
  publisher: 'Rocco',
...
const favoriteBooksFunction = () => {
  if (favoritesArray.length > 1) {
    console.log(`${reader.name} tem ${favoritesArray.length} livros favoritos.`);
  } else if(favoritesArray.length === 1) {
    console.log(`${reader.name} tem ${favoritesArray.length} livro favorito.`);
  }
}
favoriteBooksFunction();